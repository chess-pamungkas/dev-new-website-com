...
Object.keys(enLocales).forEach((key) => {
    if (!translations[key] && !key.endsWith("-accent") && enLocales[key] !== "" && !enLocales[key].startsWith("Lorem ipsum")) {
...
Object.keys(translations).forEach((key) => {
  ...
  if (translations[key].endsWith("(TRANSLATION REQUIRED!)") || translations[key].endsWith("(TR!)") || translations[key].endsWith("(TR)")) {
    missing[key] = translations[key];
  }
});

fs.writeFile(
  "src/scripts/check-missing/missing-vn.json",
