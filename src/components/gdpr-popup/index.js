...
import { GDPR_COOKIE_CATEGORIES, DEFAULT_COOKIE_CONSENT } from "../../helpers/gdpr-cookie.config";

export const GDPRPopup = ({className}) => {
    const [acceptedCookies, setAcceptedCookies] = useState(DEFAULT_COOKIE_CONSENT)
    const { handleCloseGDPRPopup, isShowGDPRPopup, acceptAllCookies, acceptCookies, handleOpenCookiePopup } = useContext(CookieContext)
          ...
          <div className="gdpr-popup__wrapper">
              <div className="gdpr-popup__content">
                  ...
                  {GDPR_COOKIE_CATEGORIES.map((cookieCategory) => (
                        <CookieCategoryItem {...cookieCategory} acceptedCookies={acceptedCookies} setAcceptedCookies={setAcceptedCookies} />
                    ))}
