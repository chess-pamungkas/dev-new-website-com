...
const NavbarSubItem = ({ className, subItem = {} }) => {
  ...
  const isItemHasSubtitles = subItem.isSubtitle && subItem.subtitles && !!subItem.subtitles.length;
      ...
      {isItemHasSubtitles && (
        <ul className="dropdown-item__subtitles">
          {subItem.subtitles.map(subtitle => (
            <li className="menu-column__item" key={`footer-menu-${stringTransformToKebabCase(subtitle.title)}`}>
              <Link
